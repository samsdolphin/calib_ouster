...
public:
  ...
  void add_residualblock(pcl::PointCloud<pcl::PointXYZRGB>::Ptr,
                         Eigen::Vector3d, Eigen::Vector3d, Eigen::Quaterniond,
                         Eigen::Vector3d);
};
...
struct p2p {
  ...
  p2p(const Eigen::Vector3d &p, const Eigen::Vector3d &pt,
  ...
  static ceres::CostFunction *Create(const Eigen::Vector3d &p,
                                     ...
                                     const Eigen::Vector3d &n) {
    return (new ceres::AutoDiffCostFunction<p2p, 1, 4, 3>(new p2p(p, pt, n)));
  }
};
